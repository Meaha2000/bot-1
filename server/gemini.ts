...
export async function chatWithGemini(
  ...
  const memories = db.prepare(`
    SELECT content, memory_type, platform 
    FROM user_memories 
    WHERE user_id = ? 
    AND (
      memory_type = 'core' 
    ...
    LIMIT 20
  `).all(userId, platform, shareActiveMemory ? 1 : 0) as { content: string, memory_type: string, platform: string }[];
        ...
        if (calls && calls.length > 0) {
          const functionResponses = await Promise.all(calls.map(async (call: any) => {
            ...
            if (call.name === 'saveMemory') {
              ...
              db.prepare('INSERT INTO user_memories (id, user_id, content, memory_type, platform) VALUES (?, ?, ?, ?, ?)').run(id, userId, content, 'active_learning', memPlatform);
              return {
                functionResponse: {
                  name: 'saveMemory',
